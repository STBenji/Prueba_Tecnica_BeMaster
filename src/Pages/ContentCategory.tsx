...
export default function ContentCategory() {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) => Math.min(prevIndex + 1, filteredMovies.length - 1))
  }
    ...
    <section className="bg-[#333] min-h-screen text-white flex items-center justify-center flex-col">
      ...
      <div className="flex gap-4">
        ...
        <div className="grid grid-flow-col gap-4">
          {filteredMovies.slice(currentIndex, currentIndex + itemsPerPage).map((item, index) => (
            <div key={index} className="relative cursor-pointer group" onClick={() => navigate(`/movies/${item.id}`)}>
              {loading && <Skeleton />}
              <img src={'https://image.tmdb.org/t/p/original' + item.poster_path} alt="" className="object-cover rounded-md" width={250} height={250} loading="lazy" onLoad={() => setLoading(false)} onError={() => setLoading(false)} />
        ...
        <button onClick={handleNext} disabled={currentIndex + itemsPerPage >= filteredMovies.length} className="cursor-pointer">
